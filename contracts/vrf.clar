(define-read-only (get-random-uint-at-height (burnblock uint))
  (let (
    (vrf-lower-uint-opt
      (match (get-burn-block-info? header-hash burnblock)
...
(define-read-only (get-uint-from-hash (hash (buff 32)))
  (buff-to-uint-le (lower-16-le hash))
)
